output$station_picker <- renderUI({
  ...
  checkboxGroupInput('station_pick', label = "Select station to graph", num_station)
})
...
output$trim_plot <- renderPlotly({
  ...
  trim_plot <- plot_ly(data = filteredData2(), type = 'scatter', mode = 'markers', x = ~Date_Time, y = ~Low_Range, key = ~(paste0(as.character(Date_Time),"_",as.character(station))), color = ~as.character(station), opacity = 0.5, source = "imgLink2") |>
